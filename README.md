...
```powershell
pyinstaller --clean -F -w --add-data font5x8.bin:. --add-data "$(python -c "import site; print(''.join(site.getsitepackages()[1]))")\*_imports.json:." --add-data res\icon.png:res --icon=res\icon.png --collect-all adafruit_blinka --runtime-tmpdir . main.py
```
For GPIO:
